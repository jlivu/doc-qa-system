...
- On the very first upload, the Qdrant collection does not yet exist —
  `upsert_chunks` creates it later in the pipeline. A `scroll()` call against
  a non-existent collection raises `qdrant_client.http.exceptions.UnexpectedResponse`.
  The `try/except` that returns `None` on a missing collection is not optional —
  without it, every first-ever upload fails before any document is stored.
...
- The `sha256` parameter is inserted between `filename` and `settings`.
  ...
  step 11.
- `chunk_size` is measured in characters, not tokens. At roughly 4
  characters per token, 800 characters ≈ 200 tokens — well within the
  OpenAI embedding model's 8,191-token limit. If `chunk_size` is ever
  raised significantly in config (e.g. to 4,000+), add a token count
...
- `zip(batch, vectors)` preserves order only if the OpenAI API returns
  embeddings in the same order as the input texts. The OpenAI API
  guarantees this, but add a defensive `assert len(vectors) == len(batch)`
  before zipping — a length mismatch would silently truncate the output
  list rather than raising an obvious error.
...
- On re-ingest, `delete_by_document_id` runs before `embed_chunks` and
  `upsert_chunks`. If deletion succeeds but a later step fails (e.g.
  `EmbeddingError`), the old chunks are gone and no new chunks have been
  stored — the document disappears from the knowledge base entirely. For
  Phase 1 this is acceptable: return HTTP 500 and let the user re-upload.
  Phase 4 can add a recovery strategy (e.g. snapshot the old document_id
  before deletion and restore on failure).
